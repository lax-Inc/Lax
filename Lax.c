...
#define banner "Lax 6.0 (6.0.6) Lax Inc. (C) Anthony Lubmansky, Mavox-ID, N467, NeedYOU7"
...
INTERFACE static int is_list(Lax *sc, pointer p);
...
#if USE_PLIST
Lax_EXPORT INLINE int hasprop(pointer p)     { return (typeflag(p)&T_SYMBOL); }
...
static int file_push(Lax *sc, const char *fname);
static void file_pop(Lax *sc);
static int file_interactive(Lax *sc);
static INLINE int is_one_of(char *s, int c);
static int alloc_cellseg(Lax *sc, int n);
static long binary_decode(const char *s);
static INLINE pointer get_cell(Lax *sc, pointer a, pointer b);
static pointer _get_cell(Lax *sc, pointer a, pointer b);
static pointer reserve_cells(Lax *sc, int n);
static pointer get_consecutive_cells(Lax *sc, int n);
static pointer find_consecutive_cells(Lax *sc, int n);
static void finalize_cell(Lax *sc, pointer a);
static int count_consecutive_cells(pointer x, int needed);
static pointer find_slot_in_env(Lax *sc, pointer env, pointer sym, int all);
static pointer mk_number(Lax *sc, num n);
static char *store_string(Lax *sc, int len, const char *str, char fill);
static pointer mk_vector(Lax *sc, int len);
static pointer mk_atom(Lax *sc, char *q);
static pointer mk_sharp_const(Lax *sc, char *name);
static pointer mk_port(Lax *sc, port *p);
static pointer port_from_filename(Lax *sc, const char *fn, int prop);
static pointer port_from_file(Lax *sc, FILE *, int prop);
static pointer port_from_string(Lax *sc, char *start, char *past_the_end, int prop);
static port *port_rep_from_filename(Lax *sc, const char *fn, int prop);
static port *port_rep_from_file(Lax *sc, FILE *, int prop);
static port *port_rep_from_string(Lax *sc, char *start, char *past_the_end, int prop);
static void port_close(Lax *sc, pointer p, int flag);
static void mark(pointer a);
static void gc(Lax *sc, pointer a, pointer b);
static int basic_inchar(port *pt);
static int inchar(Lax *sc);
static void backchar(Lax *sc, int c);
static char   *readstr_upto(Lax *sc, char *delim);
static pointer readstrexp(Lax *sc);
static INLINE int skipspace(Lax *sc);
static int token(Lax *sc);
static void printslashstring(Lax *sc, char *s, int len);
static void atom2str(Lax *sc, pointer l, int f, char **pp, int *plen);
static void printatom(Lax *sc, pointer l, int f);
static pointer mk_proc(Lax *sc, enum Lax_opcodes op);
static pointer mk_closure(Lax *sc, pointer c, pointer e);
static pointer mk_continuation(Lax *sc, pointer d);
static pointer reverse(Lax *sc, pointer a);
static pointer reverse_in_place(Lax *sc, pointer term, pointer list);
static pointer revappend(Lax *sc, pointer a, pointer b);
static void dump_stack_mark(Lax *);
static pointer opexe_0(Lax *sc, enum Lax_opcodes op);
static pointer opexe_1(Lax *sc, enum Lax_opcodes op);
static pointer opexe_2(Lax *sc, enum Lax_opcodes op);
static pointer opexe_3(Lax *sc, enum Lax_opcodes op);
static pointer opexe_4(Lax *sc, enum Lax_opcodes op);
static pointer opexe_5(Lax *sc, enum Lax_opcodes op);
static pointer opexe_6(Lax *sc, enum Lax_opcodes op);
static void Eval_Cycle(Lax *sc, enum Lax_opcodes op);
static void assign_syntax(Lax *sc, char *name);
static int syntaxnum(pointer p);
static void assign_proc(Lax *sc, enum Lax_opcodes, char *name);
...
static INLINE pointer get_cell_x(Lax *sc, pointer a, pointer b) {
...
static void push_recent_alloc(Lax *sc, pointer recent, pointer extra)
{
...
static pointer get_cell(Lax *sc, pointer a, pointer b)
{
...
static pointer get_vector_object(Lax *sc, int len, pointer init)
{
...
static INLINE void ok_to_freely_gc(Lax *sc)
{
  car(sc->sink) = sc->NIL;
}
...
static pointer oblist_initial_value(Lax *sc)
{
  return mk_vector(sc, 461);
}

static pointer oblist_add_by_name(Lax *sc, const char *name)
{
  pointer x;
  int location;
...
static INLINE pointer oblist_find_by_name(Lax *sc, const char *name)
{
  int location;
  pointer x;
  char *s;
...
static pointer oblist_all_symbols(Lax *sc)
{
  int i;
  pointer x;
...
INTERFACE pointer mk_character(Lax *sc, int c) {
  pointer x = get_cell(sc,sc->NIL, sc->NIL);
...
INTERFACE pointer mk_integer(Lax *sc, long num) {
  pointer x = get_cell(sc,sc->NIL, sc->NIL);
...
INTERFACE pointer mk_real(Lax *sc, double n) {
  pointer x = get_cell(sc,sc->NIL, sc->NIL);
...
static char *store_string(Lax *sc, int len_str, const char *str, char fill) {
     char *q;
...
INTERFACE pointer mk_string(Lax *sc, const char *str) {
     return mk_counted_string(sc,str,strlen(str));
}

INTERFACE pointer mk_counted_string(Lax *sc, const char *str, int len) {
     pointer x = get_cell(sc, sc->NIL, sc->NIL);
...
INTERFACE pointer mk_empty_string(Lax *sc, int len, char fill) {
     pointer x = get_cell(sc, sc->NIL, sc->NIL);
     typeflag(x) = (T_STRING | T_ATOM);
...
INTERFACE static pointer mk_vector(Lax *sc, int len)
...
INTERFACE pointer mk_symbol(Lax *sc, const char *name) {
     pointer x;
...
INTERFACE pointer gensym(Lax *sc) {
     pointer x;
     char name[40];
...
static port *port_rep_from_file(Lax *sc, FILE *f, int prop)
{
    port *pt;
...
static pointer port_from_file(Lax *sc, FILE *f, int prop) {
  port *pt;
  pt=port_rep_from_file(sc,f,prop);
  if(pt==0) {
    return sc->NIL;
...
static port *port_rep_from_scratch(Lax *sc) {
  port *pt;
  char *start;
  pt=(port*)sc->malloc(sizeof(port));
  if(pt==0) {
...
static pointer port_from_scratch(Lax *sc) {
  port *pt;
...
static int realloc_port_string(Lax *sc, port *p)
{
...
INTERFACE void putstr(Lax *sc, const char *s) {
...
static void putchars(Lax *sc, const char *s, int len) {
  port *pt=sc->outport->_object._port;
  if(pt->kind&port_file) {
...
INTERFACE void putcharacter(Lax *sc, int c) {
  port *pt=sc->outport->_object._port;
  if(pt->kind&port_file) {
...
static void printslashstring(Lax *sc, char *p, int len) {
  int i;
...
static pointer list_star(Lax *sc, pointer d) {
  pointer p, q;
  if(cdr(d)==sc->NIL) {
    return car(d);
  }
...
static void new_frame_in_env(Lax *sc, pointer old_env)
{
  pointer new_frame;
...
static INLINE void new_slot_spec_in_env(Lax *sc, pointer env,
...
static pointer find_slot_in_env(Lax *sc, pointer env, pointer hdl, int all)
{
  pointer x,y;
  int location;
...
static INLINE void new_frame_in_env(Lax *sc, pointer old_env)
{
...
static INLINE void new_slot_in_env(Lax *sc, pointer variable, pointer value)
{
  new_slot_spec_in_env(sc, sc->envir, variable, value);
}

static INLINE void set_slot_in_env(Lax *sc, pointer slot, pointer value)
{
  cdr(slot) = value;
}
...
static pointer _Error_1(Lax *sc, const char *s, pointer a) {
     const char *str = s;
#if USE_ERROR_HOOK
     pointer x;
...
static void s_save(Lax *sc, enum Lax_opcodes op, pointer args, pointer code)
{
...
static pointer _s_return(Lax *sc, pointer a)
{
  int nframes = (int)sc->dump;
...
static INLINE void dump_stack_reset(Lax *sc)
{
  sc->dump = (pointer)0;
}

static INLINE void dump_stack_initialize(Lax *sc)
{
  sc->dump_size = 0;
  sc->dump_base = NULL;
  dump_stack_reset(sc);
...
static void dump_stack_free(Lax *sc)
{
  free(sc->dump_base);
  sc->dump_base = NULL;
  sc->dump = (pointer)0;
...
static INLINE void dump_stack_mark(Lax *sc)
{
  int nframes = (int)sc->dump;
  int i;
  for(i=0; i<nframes; i++) {
...
static pointer opexe_0(Lax *sc, enum Lax_opcodes op) {
     ...
     case OP_T1LVL:
          ...
          if (is_symbol(sc->code)) {
              char *s = symname(sc->code);
              if (strcmp(s, "help") == 0 || strcmp(s, "exit") == 0 || strcmp(s, "quit") == 0) {
                  sc->code = cons(sc, sc->code, sc->NIL); 
              }
          }
    ...
    if(file_interactive(sc)) {
     ...
     case OP_E0ARGS:
          if (is_macro(sc->value)) {
               s_save(sc,OP_DOMICRO, sc->NIL, sc->NIL);
     ...
     case OP_AST:
          {
               ...
               if(f==sc->NIL) {
                    ...
                    s_save(sc,OP_AST1,sc->args,sc->code);
                    sc->args=cons(sc,sc->code,sc->NIL);
     ...
     case OP_LET2AST:
      ...
      case OP_INPUT: {
          ...
          if (sc->args != sc->NIL && is_string(car(sc->args))) {
              p_str = string_value(car(sc->args));
          } else {
              p_str = prompt;
          }

          if (sc->outport->_object._port->kind & port_file) {
              fflush(sc->outport->_object._port->rep.stdio.file);
          }

          line = linenoise(p_str);

          if (line != NULL) {
              pointer s = mk_string(sc, line);
              linenoiseFree(line);
              s_return(sc, s);
          } else {
              s_return(sc, sc->EOF_OBJ);
...
case OP_TARRY: {
     double val = rvalue(car(sc->args));
     unsigned long ms = (unsigned long)val;
     ...
     case OP_SETHEAD:
       if(!is_immutable(car(sc->args))) {
         ...
         Error_0(sc,"set-head!: unable to alter immutable pair");
       }
...
static int is_list(Lax *sc, pointer a)
...
case OP_IMAGE:
    ...
    if (is_string(x)) {
        ...
        for (i = 0; i < len; i++) {
            if (str[i] == '\\' && i + 1 < len) {
                switch (str[i + 1]) {
                    case 'n': putcharacter(sc, '\n'); i++; break;
                    case 't': putcharacter(sc, '\t'); i++; break;
                    case 'r': putcharacter(sc, '\r'); i++; break;
                    case '\\': putcharacter(sc, '\\'); i++; break;
                    default: putcharacter(sc, str[i]); break;
                }
            } else {
                putcharacter(sc, str[i]);
            }
        }
    ...
    s_goto(sc,OP_P0LIST);
     ...
     case OP_ERR0:
          ...
          Error_1(sc, strvalue(car(sc->args)), 0); 
     ...
     case OP_NEWSEGMENT:
          if (!is_pair(sc->args) || !is_number(car(sc->args))) {
               Error_0(sc,"Error: argument must be a number");
          }
...
static pointer opexe_5(Lax *sc, enum Lax_opcodes op) {
    ...
    if(is_vector(sc->args)) {
        ...
        if(sc->interactive_repl) { putstr(sc, "\n"); }
        s_return(sc,sc->T);
...
typedef pointer (*dispatch_func)(Lax *, enum Lax_opcodes);
...
static void Eval_Cycle(Lax *sc, enum Lax_opcodes op) {
  ...
  for (;;) {
    ...
    ok_to_freely_gc(sc);
    if (pcd->func(sc, (enum Lax_opcodes)sc->op) == sc->NIL) {
      return;
    }
    if(sc->no_memory) {
...
static void assign_proc(Lax *sc, enum Lax_opcodes op, char *name) {
     pointer x, y;
...
static int syntaxnum(pointer p) {
     ...
     case 3:
          if(s[0]=='a') {
              if (s[1]=='s') return OP_AST;
              return OP_AND0;
          }
          return OP_LET0;
     case 4:
          if(strcmp(s, "spot") == 0) return OP_DEF0;
          switch(s[3]) {
          case 'e': return OP_CASE0;
          case 'd': return OP_COND0;
          case '*': return OP_LET0AST;
          case 't': return OP_DEF0;
          default: return OP_SET0;
          }
     case 5:
          if(strcmp(s, "chlet") == 0) return OP_COND0;
          if(strcmp(s, "start") == 0) return OP_BEGIN;
          if(strcmp(s, "micro") == 0) return OP_MACRO0;
          if(strcmp(s, "cite") == 0) return OP_QUOTE;
          switch(s[2]) {
          case 'g': return OP_BEGIN;
          case 'l': return OP_TARRY;
          case 'c': return OP_MACRO0;
          default: return OP_QUOTE;
          }
     case 6:
          if(strcmp(s, "ast") == 0) return OP_AST;
          switch(s[2]) {
          case 'm': return OP_AST;
...
#if USE_INTERFACE
INTERFACE static pointer s_cons(Lax *sc, pointer a, pointer b) {
 return cons(sc,a,b);
}
INTERFACE static pointer s_immutable_cons(Lax *sc, pointer a, pointer b) {
 return immutable_cons(sc,a,b);
}

static struct Lax_interface vtbl ={
  Lax_define,
  s_cons,
  s_immutable_cons,
  reserve_cells,
...
static pointer
lax_help(Lax *sc, pointer args)
{
    (void) args;

    puts("Available commands:");
    puts("");
    puts("Regular commands:");
    puts("  image         - Print a line to the current output");
    puts("  say           - Print a line with "" to the current output and return #+");
    puts("  exit          - Exit REPL and return to shell");
    puts("  help          - Show this help text");
    puts("  nl            - Call a new line (Wrap)");
    puts("  err           - Raise an error with your message");
    puts("");
    puts("Conditional commands:");
    puts("  if            - Condition if ...");
    puts("  start         - Execute multiple statements in a row");
    puts("  spot          - Create a global or local name-to-value binding");
    puts("  and           - output a newline on the current output port");
    puts("  or            - output a newline on the current output port");
    puts("  =             - Compare the first and second values");
    puts("  <             - The second value is greater than the first");
    puts("  >             - The first value is greater than the second");
    puts("  <=            - The first and second values ​​are equal or the second value is greater");
    puts("  >=            - The first and second values ​​are equal or the first value is greater");
    puts("");
    puts("Commands for working with data:");
    puts("  ast           - Create an executable object that can be called later");
    puts("  cite          - Prevent expression evaluation");
    puts("  set$          - Edit an existing binding");
    puts("  let           - Create a temporary environment with new bindings");
    puts("  let$          - Create a chain of dependent bindings");
    puts("  let*          - Let bindings reference each other");
    puts("  chlet         - Selects the first suitable branch");
    puts("  chlet-stream  - Create a lazy pair");
    puts("  micro         - Changes code before execution");
    puts("  syntax-rules  - pattern macros > likely absent");
    puts("  {quasiquote                                              ");
    puts("  {unquote      - Template construction (`), used by macros");
    puts("  {unquote-splicing                                        ");
    puts("  choose        - Selects a branch by value");
    puts("");
    puts("I/O and ports");
    puts("  read          - Read one Lax expression from current input");
    puts("  head          - First element of pair/list");
    puts("  tail          - Tail (rest) of pair/list");
    puts("  read-char     - Read next character from current input port");
    puts("  peek-char     - Peek next char without consuming it");
    puts("  say-char      - Say a single character to output");
    puts("  eof-object?   - Test for end-of-file marker");
    puts("  open-i/o-file - Open a file as input/output port");
    puts("  close-i/o-port- Close port");
    puts("  load          - Load and evaluate expressions from a file");
    puts("  load-extension- Load shared library (dynamic extension)");
    puts("current-input-port- Get current input");
    puts("current-output-port- Get current output");
    puts("  set-input-port - Change current input");
    puts("  set-output-port- Change current output");
    puts("open-input-string- Create port from string");
    puts("get-output-string- Get string from accumulated port");
    puts("");
    puts("System:");
    puts("  system        - Execute a system command, return status/output (if present)");
    puts("  file-exists?  - Test file presence");
    puts("  gc            - Force garbage collection");
    puts("  gc-verbose    - Toggle GC verbosity");
    puts("  oblist        - Return list of all interned symbols");
    puts("  new-segment   - Allocate new memory segment");
    puts("  defined?      - Check if a symbol is defined");
    puts("  quit          - Immediately terminate the REPL and subsequent execution");
    puts("");
    puts("Call and functions:");
    puts("  apply         - Apply procedure to list of args");
    puts("  eval          - Evaluate Lax expression in current environment");
    puts("  map           - Map function over list(s), return new list");
    puts("  for-each      - Apply function to list elements for side-effects (no result list)");
    puts("--- Low-level/auxiliary (functions and commands) ---");
    puts(" apply-primitive- Low-level/auxiliary function");
    puts("  compose       - Low-level/auxiliary command");
    puts("");
    puts("Lists:");
    puts("  cons          - Create pair from two values");
    puts("  list*         - Create improper list (last arg is tail)");
    puts("  reverse       - Reverse a list");
    puts("  append        - Concatenate lists");
    puts("  head          - First element (car)");
    puts("  tail          - Rest of list (cdr)");
    puts("  set-head!     - Modify the head (car) of a pair");
    puts("  set-tail!     - Modify the tail (cdr) of a pair");
    puts("  assq          - Find pair by key (using eq?)");
    puts("  memq, memv    - Membership tests");
    puts("");
    puts("Type Tests:");
    puts("  number?       - Test number");
    puts("  integer?      - Test integer");
    puts("  sign?         - Test symbol");
    puts("  string?       - Reverse a list");
    puts("  boolean?      - Test for empty list");
    puts("  {char?                         ");
    puts("  {char=?       - char predicates");
    puts("  {char<?                        ");
    puts("  procedure?    - Test if value is callable");
    puts("  port?         - Test whether object is port");
    puts("  list?         - Test if value is a proper list");
    puts("  vector?       - Test if value is a vector");
    puts("  environment?  - Test if value is an environment");
    puts("  input-port?   - Test for input port");
    puts("  output-port?  - Test for output port");
    puts("  closure?      - Test if value is a closure");
    puts("  micro?        - Test if value is a macro");
    puts("");
    puts("String operations:");
    puts("  string-length - Length of string");
    puts("  string-ref    - Char at index");
    puts("  string-set!   - Mutate char in string");
    puts("  substring     - Extract substring");
    puts("  string-append - Concatenate strings");
    puts("  make-string   - Create a string of defined length");
    puts("  char->integer - Convert char to ASCII code");
    puts("  integer->char - Convert code to char");
    puts("  char-upcase   - Convert char to upper case");
    puts("  char-downcase - Convert char to lower case");
    puts("char-alphabetic?- Is char a letter?");
    puts("  char-numeric? - Is char a digit?");
    puts("char-whitespace?- Is char a space/tab/nl?");
    puts("{string->symbol |}");
    puts("{symbol->string - Conversions");
    puts("-");
    puts("  {string=?     |}");
    puts("  {string<?     - String compares");
    puts("");
    puts("Symbols:");
    puts("  symbol->string- Convert symbol to string");
    puts("  string->symbol- Convert string to symbol");
    puts("  gensym        - Generate new unique symbol");
    puts("");
    puts("Logic:");
    puts("  not           - Logical negation");
    puts("  {eq?                                       ");
    puts("  {eqv?         - Different equality notions:");
    puts("  {equal?                                    ");
    puts("  eq?           - identity (pointer-level)");
    puts("  eqv?          - like eq? but for numbers/booleans treats equal values");
    puts("  equal?        - deep structural equality (lists, strings...)");
    puts("");
    puts("Arithmetic functions:");
    puts("  +             - Add one value to another");
    puts("  -             - Subtract one value from another");
    puts("  *             - Multiply one value by another");
    puts("  /             - Divide one value by another");
    puts("-");
    puts("  {abs          |}               ");
    puts("  {quotient     - integer helpers");
    puts("  {remainder    |}               ");
    puts("  {modulo       |}               ");
    puts("-");
    puts("  {floor        |}        ");
    puts("  {ceiling      - rounding");
    puts("  {round        |}        ");
    puts("  {truncate     |}        ");
    puts("  max           - Maximum");
    puts("  min           - Minimum");
    puts("");
    puts("Math & Trigonometry:");
    puts("  exp, log      - Exponential and natural logarithm");
    puts("  sin, cos, tan - Trigonometric functions");
    puts("  asin, acos    - Arc trigonometric functions");
    puts("  atan          - Arc tangent");
    puts("  sqrt          - Square root");
    puts("  expt          - Exponentiation (power)");
    puts("  inexact->exact- Convert float to integer");
    puts("Vector");
    puts("  vector        - Create vector from args");
    puts("  make-vector   - Create vector of length n w/ initial value");
    puts("  vector-ref    - Read vector element");
    puts("  vector-set!   - Write vector element");
    puts("  vector-length - Length");
    puts("");
    puts("Streams:");
    puts("  tarry         - Wait for the specified time before executing");
    puts("  stream        - helpers > map/for-each variants for streams");
    puts("  force         - Evaluate a delayed promise");
    puts("");
...
static pointer
lax_exit(Lax *sc, pointer args)
{
    (void) sc;
    (void) args;
    
    puts("Thank you for using Lax 6.0!");
    sleep(1);
    puts("Bye!");
    sleep(1);
    fflush(stdout);
...
Lax *Lax_init_new_custom_alloc(func_alloc malloc, func_dealloc free) {
  Lax *sc=(Lax*)malloc(sizeof(Lax));
  if(!Lax_init_custom_alloc(sc,malloc,free)) {
    free(sc);
    return 0;
  } else {
    return sc;
...
int Lax_init(Lax *sc) {
 return Lax_init_custom_alloc(sc,malloc,free);
}

int Lax_init_custom_alloc(Lax *sc, func_alloc malloc, func_dealloc free) {
  ...
  assign_syntax(sc, "ast");
  assign_syntax(sc, "cite");
  assign_syntax(sc, "spot");
  assign_syntax(sc, "if");
  assign_syntax(sc, "start");
  assign_syntax(sc, "set$");
  assign_syntax(sc, "let");
  assign_syntax(sc, "let$");
  assign_syntax(sc, "let*");
  assign_syntax(sc, "chlet");
  assign_syntax(sc, "and");
  assign_syntax(sc, "or");
  assign_syntax(sc, "chlet-stream");
  assign_syntax(sc, "micro");
  assign_syntax(sc, "choose");

  for(i=0; i<n; i++) {
    if(dispatch_table[i].name!=0) {
      assign_proc(sc, (enum Lax_opcodes)i, dispatch_table[i].name);
    }
  }
  
Lax_define(sc, sc->global_env, mk_symbol(sc, "help"), mk_foreign_func(sc, lax_help));
Lax_define(sc, sc->global_env, mk_symbol(sc, "exit"), mk_foreign_func(sc, lax_exit));
Lax_define(sc, sc->global_env, mk_symbol(sc, "quit"), mk_foreign_func(sc, lax_quit));

  sc->LAMBDA = mk_symbol(sc, "ast");
  sc->QUOTE = mk_symbol(sc, "cite");
...
void Lax_set_output_port_file(Lax *sc, FILE *fout) {
...
void Lax_load_file(Lax *sc, FILE *fin)
{ Lax_load_named_file(sc,fin,0); }

void Lax_load_named_file(Lax *sc, FILE *fin, const char *filename) {
  dump_stack_reset(sc);
  sc->envir = sc->global_env;
  sc->file_i=0;
  
  sc->load_stack[0].kind = port_input | port_file; 
  sc->load_stack[0].kind &= ~port_saw_EOF; 
  
  sc->load_stack[0].rep.stdio.file = fin;
  sc->load_stack[0].rep.stdio.closeit = 0;
...
void Lax_load_string(Lax *sc, const char *cmd) {
  dump_stack_reset(sc);
  sc->envir = sc->global_env;
  sc->file_i=0;
...
void Lax_define(Lax *sc, pointer envir, pointer symbol, pointer value) {
     pointer x;
...
#if !STANDALONE
void Lax_register_foreign_func(Lax * sc, Lax_registerable * sr)
{
  Lax_define(sc,
                sc->global_env,
                                       ...
                                       int count)
{
  ...
  for(i = 0; i < count; i++)
    {
      Lax_register_foreign_func(sc, list + i);
    }
}

pointer Lax_apply0(Lax *sc, const char *procname)
{ return Lax_eval(sc, cons(sc,mk_symbol(sc,procname),sc->NIL)); }

void save_from_C_call(Lax *sc)
{
  pointer saved_data =
    cons(sc,
         car(sc->sink),
...
void restore_from_C_call(Lax *sc)
{
...
pointer Lax_call(Lax *sc, pointer func, pointer args)
{
...
pointer Lax_eval(Lax *sc, pointer obj)
{
  int old_repl = sc->interactive_repl;
  sc->interactive_repl = 0;
  save_from_C_call(sc);
...
void print_footer(const char *progname, const char *version) {
    int use_color = isatty(fileno(stdout));

    const char *C_RST  = use_color ? "\x1b[0m"  : "";
    const char *C_HDR  = use_color ? "\x1b[1;36m" : "";
    const char *C_KEY  = use_color ? "\x1b[1;33m" : "";
    const char *C_OPT  = use_color ? "\x1b[0;37m" : "";
    ...
    snprintf(buf, sizeof(buf), "|   %sLax%s — Ultra lightweight language     |\n", C_KEY, C_HDR);
    type_print(buf);
    ...
    snprintf(buf, sizeof(buf), "%s%s %s\n\n", C_HDR, progname, version);
    type_print(buf);

    snprintf(buf, sizeof(buf), "%sUsage:%s\n", C_KEY, C_RST);
    type_print(buf);

    type_print("  -i              : Interactive Mode (REPL)\n");
    type_print("  -o <file.lx>    : Run a script file (interpreter)\n");
    type_print("  -c <file.lx>    : Compile .c and binary program\n\n");

    snprintf(buf, sizeof(buf), "%sMore:%s\n", C_KEY, C_RST);
    type_print(buf);

    type_print("  Authors : Anthony Lubmansky, Mavox-ID, N467, NeedYOU7\n");
    type_print("  Company : https://github.com/lax-Inc\n");
    type_print("  Repo    : https://github.com/lax-Inc/Lax\n\n");
    
}

const char *PAYLOAD_MARKER = "####LAX_DCODE####";

void minify_lax_code(char *code) {
    char *src = code;
    char *dst = code;
    int in_string = 0;
    int in_comment = 0;
...
int load_embedded_script(Lax *sc, const char *exe_path) {
    ...
    char *payload = NULL;
    size_t marker_len = strlen(PAYLOAD_MARKER);
    
    for (long i = (long)read_bytes - (long)marker_len - 1; i >= 0; i--) {
        if (memcmp(data + i, PAYLOAD_MARKER, marker_len) == 0) {
            payload = data + i + marker_len;
            break;
        }
    }
    
    if (payload && *payload) {
        Lax_load_string(sc, (const char*)init_lx); 
        
        Lax_load_string(sc, payload);
...
int main() {
     extern int MacTS_main(int argc, char **argv);
     char** argv;
     int argc = ccommand(&argv);
     return MacTS_main(argc,argv);
}
...
int main(int argc, char **argv) {
  ...
  Lax_set_input_port_file(&sc, stdin);
  Lax_set_output_port_file(&sc, stdout);

  if (load_embedded_script(&sc, argv[0])) {
    retcode = sc.retcode;
    Lax_deinit(&sc);
    return retcode;
  }
  ...
  if(argc == 1) {
    print_footer("Lax", "6.0 (6.0.6) Lax Inc. (C) Anthony Lubmansky, Mavox-ID, N467, NeedYOU7");
    Lax_deinit(&sc);
    return 0;
  }
  ...
  if(strcmp(argv[1], "-i") == 0) {
      ...
      printf("Lax 6.0 (6.0.6) Lax Inc. (C) Anthony Lubmansky, Mavox-ID, N467, NeedYOU7\n");
      printf("Type 'help' for a menu of available commands\n\n");
      
      char *line;
      linenoiseHistoryLoad(".lax_history");
      linenoiseSetMultiLine(1);

      while((line = linenoise("Lax> ")) != NULL) {
          if (line[0] != '\0') {
              linenoiseHistoryAdd(line);
              linenoiseHistorySave(".lax_history");
              Lax_load_string(&sc, line);
          }
          linenoiseFree(line);
      }
  } 
    ...
    if(argc < 3) {
        fprintf(stderr, "Error: filename required for -o\n");
        Lax_deinit(&sc);
        return 1;
    }
    ...
    fin = fopen(argv[2], "r");
    if(fin) {
        Lax_load_named_file(&sc, fin, argv[2]);
        fclose(fin);
    } else {
      ...
      FILE *fp_src, *fp_exe, *fp_out;
      char out_name[1024];
      char script_buf[65536];
      size_t script_len, exe_len;
      char *exe_data;

      if(argc < 3) {
          fprintf(stderr, "Error: filename required for -c\n");
          Lax_deinit(&sc);
          return 1;
      }
      ...
      script_len = fread(script_buf, 1, sizeof(script_buf) - 1, fp_src);
      script_buf[script_len] = '\0';
      fclose(fp_src);

      minify_lax_code(script_buf);

      printf("Compiling %s -> binary '%s'...\n", argv[2], out_name);

      fp_exe = fopen(argv[0], "rb");
      if(!fp_exe) {
          fprintf(stderr, "Cannot read Lax executable\n");
          Lax_deinit(&sc);
          return 1;
      }
      
      fseek(fp_exe, 0, SEEK_END);
      exe_len = ftell(fp_exe);
      fseek(fp_exe, 0, SEEK_SET);
      ...
      if (fread(exe_data, 1, exe_len, fp_exe) != exe_len) {
          fprintf(stderr, "Error reading executable data\n");
      }
      fclose(fp_exe);

      fp_out = fopen(out_name, "wb");
      if(!fp_out) {
          fprintf(stderr, "Cannot create binary %s\n", out_name);
          free(exe_data);
          Lax_deinit(&sc);
          return 1;
      }

      fwrite(exe_data, 1, exe_len, fp_out);
      fprintf(fp_out, "%s", PAYLOAD_MARKER);
      fwrite(script_buf, 1, strlen(script_buf), fp_out);
      ...
      printf("Success! Generated binary: %s\n", out_name);
      Lax_deinit(&sc);
      return 0;
  }
  else {
